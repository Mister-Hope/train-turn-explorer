...
const StatusCard: React.FC<StatusCardProps> = ({ status, diff }) => {
  const config = {
    perfect: {
      ...
      icon: <CheckCircle2 className="w-10 h-10 text-green-700" />,
      title: "完美状态",
    ...
    slow: {
      ...
      icon: <AlertOctagon className="w-10 h-10 text-orange-700" />,
      title: "速度过慢",
      desc: "火车有近心趋势，轮缘挤压内轨 (内轨提供指向外侧的弹力)。",
    },
    stopped: {
      color: "bg-slate-200 text-slate-800 border-slate-400",
      icon: <PauseCircle className="w-10 h-10 text-slate-700" />,
      title: "静止状态",
  ...
  return (
    <div
      className={`p-6 rounded-xl border-l-8 shadow-md flex items-center gap-5 transition-all duration-300 ${current.color} border-${current.color.split(" ")[2].replace("border-", "")}`}
    >
      <div className="flex-shrink-0">{current.icon}</div>
      <div>
        <h3 className="text-2xl font-bold mb-2">{current.title}</h3>
        <p className="text-lg opacity-90 leading-relaxed">{current.desc}</p>
      </div>
    </div>
  );
};
